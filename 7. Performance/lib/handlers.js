    ...
    _data.read('users', phone, function(err, userData) {
      ...
      if (!err && userData) {
        ...
        if (hashedPassword == userData.hashedPassword) {
          ...
          _performance.mark('beginning create data for token')
          ...
          _performance.mark('beginning token storing')

          // Store the token
          _data.create('tokens', tokenId, tokenObject, function(err) {
            _performance.mark('token stored')

            // Gather all the measurements
            _performance.measure('Beginning to end', 'entered function', 'token stored')
            _performance.measure('Validating user input', 'entered function', 'inputs validated')
            _performance.measure('User lookup', 'beginning user lookup', 'user lookup competed')
            _performance.measure('Password hashing', 'beginning password hashing', 'competed password hashing')
            _performance.measure('Token storing', 'beginning token storing', 'token stored')
        ...
        _data.read('users', userPhone, function(err, userData) {
          if (!err && userData) {
            ...
            if (userChecks.length < config.maxChecks) {
              ...
              // Verify that the URL given has DNS entries and there fore can resolve
              var parseUrl = _url.parse(protocol + '://' + url, true)
              var hostName = typeof(parseUrl.hostname) == 'string' && parseUrl.hostname.length > 0 ? parseUrl : false
              dns.resolve(hostName, function(err, records) {
                if (!err && records) {
                  ...
                  // Create check object including userPhone. Let's check if there's a need for it!
                  var checkObject = {
                    'id': checkId,
                    'userPhone': userPhone,
                    'protocol': protocol,
                  ...
                  calbback(500, {
                    'Error': 'The hostname of the URL entere did not resolve'
                  })
                }
              })
            } else {
